...
class PhaseTypeShapeArrow extends PhaseType {
	...
	override def instantiate(phaseManager: PhaseManager): Phase = new PhaseArrow(phaseManager, ShotData.DefaultShotData, MovementData.constant(0.4D),
		RotationData.none, 5, 1D, 1D, this)

	def instantiate(phaseManager: PhaseManager, shotData: ShotData, movementData: MovementData, rotationData: RotationData, amount: Int,
			distance: Double, width: Double): Phase = new PhaseArrow(phaseManager, shotData, movementData, rotationData, amount, distance, width, this)
}
...
class PhaseArrow(
		...
		var distance: Double,
		var width: Double,
		val getType: PhaseTypeShapeArrow) extends Phase(manager) {
