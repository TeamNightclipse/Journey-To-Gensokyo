...
public class THSC_FinalSpark extends THSpellCard {
	...
	public void spellcard_main() {
		...
		if (time >= 30 && time <= 100) {
			...
			double userYawSin = MathHelper.sin((float)userYawRad);
			double userYawCos = MathHelper.cos((float)userYawRad);
			double userPitchSin = MathHelper.sin((float)userPitchRad);
			...
			for (int i = 0; i < pieces; i++) {
				...
				yVector = -angleXZCos * MathHelper.sin((float)(userPitchRad - angleYRad));
